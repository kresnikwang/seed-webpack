    ...
    class BrowserService
      ...
      get: ()->
        ...
        if @mobileAndTabletCheck()
          browser = "mobile"
          if @checkWechat() || ShareService.inWechat
            browser = "wechat"
      ...
      checkIOS: ()=>
        return /iPad|iPhone|iPod/.test(navigator.userAgent) and !window.MSStream

      checkWechat: ()=>
        return (navigator.userAgent?.toLowerCase().indexOf('micromessenger') > -1) or (typeof navigator.wxuserAgent != "undefined")

    window.BrowserService = new BrowserService()
]